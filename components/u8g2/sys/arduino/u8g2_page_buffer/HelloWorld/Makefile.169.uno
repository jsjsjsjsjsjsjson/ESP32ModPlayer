...
#	005	09 Aug 2015	Arduino 1.6.5
#	006	11 May 2016	Arduino 1.6.9
#
...
ARDUINO_PATH:=/home/kraus/prg/arduino-1.6.9/
...
EXTRA_DIRS+=$(ARDUINO_PATH)hardware/arduino/avr/libraries/SPI/src/
EXTRA_DIRS+=$(ARDUINO_PATH)hardware/arduino/avr/libraries/Wire/src/
...
CDIRS:=*.c utility/*.c $(U8G_PATH)*.c $(addsuffix *.c,$(CDIRS)) $(ARDUINO_PATH)hardware/arduino//avr/cores/arduino/*.c
...
COMMON_FLAGS = -DF_CPU=$(F_CPU) -mmcu=$(MCU) $(DEFS) -DARDUINO=169
# COMMON_FLAGS += -gdwarf-2
COMMON_FLAGS += -Os
COMMON_FLAGS += -Wall -funsigned-char -funsigned-bitfields -fpack-struct -fshort-enums
COMMON_FLAGS += -I.
COMMON_FLAGS += -I$(ARDUINO_PATH)hardware/arduino/avr/cores/arduino
COMMON_FLAGS += -I$(ARDUINO_PATH)hardware/arduino/avr/variants/$(VARIANT)
...
$(TMPDIRPATH)%.cpp: %.ino
	@cat $(ARDUINO_PATH)hardware/arduino/avr/cores/arduino/main.cpp > $@
	@cat $< >> $@
	@echo >> $@
