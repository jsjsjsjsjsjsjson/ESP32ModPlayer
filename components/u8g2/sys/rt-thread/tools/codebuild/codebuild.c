...
struct interface
{
  ...
  const char *rtthread_com_procedure;	/* u8x8_byte_4wire_sw_spi */
  const char *rtthread_gpio_procedure;	/* u8x8_gpio_and_delay_rtthread */
...
void do_u8x8_header(int controller_idx, int display_idx, int interface_idx)
{
  ...
  // Only use HW SPI when available in RT-Thread settings
  if (interface_idx == 1)
  {
    fprintf(fp, "#if defined U8G2_USE_HW_SPI\n");
  }

  // Only use HW I2C when available in RT-Thread settings
  if (interface_idx == 7)
  {
    fprintf(fp, "#if defined U8G2_USE_HW_I2C\n");
  }
  ...
  fprintf(fp, "%s, ", interface_list[interface_idx].rtthread_com_procedure);  
  fprintf(fp, "%s);\n", interface_list[interface_idx].rtthread_gpio_procedure);  
  fprintf(fp, "    ");
...
class U8G2_SSD1306_128x64_NONAME_1_SW_SPI : public U8G2
{
  public:
    ...
    {
      u8g2_Setup_ssd1306_128x64_noname_1(&u8g2, rotation, u8x8_byte_4wire_sw_spi, u8x8_gpio_and_delay_rtthread,);
      u8x8_SetPin_4Wire_SW_SPI(getU8x8(), clock, data, cs, dc, reset);
    }
};
*/
...
void do_display(int controller_idx, int display_idx, const char *postfix)
{
  ...
  {
    /* currently there is no RT-Thread C++ interface, instead the interface is created manually in the example */
  }
...
int main(void)
{
  ...
  insert_into_file("../../cppsrc/U8g2lib.h", "U8g2lib.h", "/* RT-Thread constructor list start */", "/* RT-Thread constructor list end */");
